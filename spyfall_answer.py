...
### Setup ###
# Create Bot
bot = TBot('827940689:AAGSKUyy4ZB8rS_UsMN_YcmrV2ZZe7hAKGQ')
...
def create_new_game(user_id, duration):
    ...
    bot.send_message(user_id, room_id)
...
def join_game(user_id, room_id):
    ...
    if room_id not in game_rooms:
        bot.send_message(user_id, 'Game Room does not exist')
        return
    ...
    bot.send_message(user_id, 'You are in the game!')
...
def start_game(user_id):
    ...
    # What if the room does not exist?
    if room_id not in game_rooms:
        bot.send_message(user_id, 'No Game Room Found. Only the game leader can start the game.')
        return
    ...
    location = random.choice(list(location_roles))

    # Get the roles of that location
    # Store variable as "roles"
    roles = location_roles[location]
    ...
    players = game_room['players']
    player_roles = random.sample(roles, len(players) - 1)

    # Add Spy into "player_roles"
    player_roles.append('Spy')
    ...
    location_message = 'Locations: \n' + '\n'.join(list(location_roles))
    for user_id in players:
        bot.send_message(user_id, location_message)

    for user_id in players:
        bot.send_message(user_id, 'Game starts in ...')

    for i in range(3, 0, -1):
        for chat_id in players:
            bot.send_message(chat_id, f'{i}...')

    bot.send_message(players[-1], 'You are the SPY!')

    for player_id, role in zip(players[:-1], player_roles[:-1]):
        bot.send_message(player_id, 'You are NOT the Spy!')
        bot.send_message(player_id, f'Location: {location}')
        bot.send_message(player_id, f'Role: {role}')
...
def end_game(user_id):
    ...
    if room_id not in game_rooms:
        bot.send_message(user_id, 'Game Room does not exist. Only the game leader can stop the game.')
        return
    ...
    game_room = game_rooms[room_id]
    players = game_room['players']
    roles = game_room['roles']
    location = game_room['location']

    for player_id in players:
        bot.send_message(player_id, 'Results in...')

    for i in range(3, 0, -1):
        for player_id in players:
            bot.send_message(player_id, f'{i}...')
    ...
    for player_id in players:
        bot.send_message(player_id, results)
